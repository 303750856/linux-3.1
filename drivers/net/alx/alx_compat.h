...
#ifndef _ALX_COMPAT_H_
#define _ALX_COMPAT_H_
...
/* NAPI enable/disable state here */
/* enable NAPI for alx by default */
#undef CONFIG_ALX_NAPI
//#define CONFIG_ALX_NAPI
//#define NAPI
...
#define adapter_struct alx_adapter
#define adapter_msix_param alx_msix_param
...
#define netif_msg_drv(p)	((p)->msg_flags & NETIF_MSG_DRV)
#define netif_msg_probe(p)	((p)->msg_flags & NETIF_MSG_PROBE)
#define netif_msg_link(p)	((p)->msg_flags & NETIF_MSG_LINK)
#define netif_msg_timer(p)	((p)->msg_flags & NETIF_MSG_TIMER)
#define netif_msg_ifdown(p)	((p)->msg_flags & NETIF_MSG_IFDOWN)
#define netif_msg_ifup(p)	((p)->msg_flags & NETIF_MSG_IFUP)
#define netif_msg_rx_err(p)	((p)->msg_flags & NETIF_MSG_RX_ERR)
#define netif_msg_tx_err(p)	((p)->msg_flags & NETIF_MSG_TX_ERR)
#define netif_msg_tx_queued(p)	((p)->msg_flags & NETIF_MSG_TX_QUEUED)
#define netif_msg_intr(p)	((p)->msg_flags & NETIF_MSG_INTR)
#define netif_msg_tx_done(p)	((p)->msg_flags & NETIF_MSG_TX_DONE)
#define netif_msg_rx_status(p)	((p)->msg_flags & NETIF_MSG_RX_STATUS)
#define netif_msg_pktdata(p)	((p)->msg_flags & NETIF_MSG_PKTDATA)
#else /* !HAVE_NETIF_MSG && < 3.1.0 */
#define NETIF_MSG_HW	0x2000
#define NETIF_MSG_WOL	0x4000
#endif /* HAVE_NETIF_MSG */
#ifndef netif_msg_hw
#define netif_msg_hw(p)		((p)->msg_flags & NETIF_MSG_HW)
#endif
#ifndef netif_msg_wol
#define netif_msg_wol(p)	((p)->msg_flags & NETIF_MSG_WOL)
#endif
...
#define pci_set_mwi(X) pci_write_config_word(X, \
			       PCI_COMMAND, adpt->hw.bus.pci_cmd_word | \
...
#define spin_trylock_irqsave(tx_lock, flags) \
	spin_trylock(tx_lock)
#else
static inline struct device *pci_dev_to_dev(struct pci_dev *pdev)
{
	return &pdev->dev;
...
#ifndef CONFIG_ALX_DISABLE_PACKET_SPLIT
#define CONFIG_ALX_DISABLE_PACKET_SPLIT 1
#endif
...
#ifndef PCI_DEV_FLAGS_MSI_INTX_DISABLE_BUG
#define PCI_DEV_FLAGS_MSI_INTX_DISABLE_BUG 0
#endif
#define SET_PCI_DEV_FLAG(_flag)
#else /* 2.6.22 */
#define ETH_TYPE_TRANS_SETS_DEV
#define HAVE_NETDEV_STATS_IN_NETDEV
#define SET_PCI_DEV_FLAG(_flag)		pdev->dev_flags |= (_flag)
#endif /* < 2.6.22 */
...
#define device_set_wakeup_enable(dev, val) \
	do { \
		u16 pmc = 0; \
		int pm = pci_find_capability(adpt->pdev, PCI_CAP_ID_PM); \
		if (pm) { \
			pci_read_config_word(adpt->pdev, pm + PCI_PM_PMC, \
				&pmc); \
		} \
...
do {								\
	struct adapter_struct *kc_adpt = netdev_priv(netdev);\
	struct pci_dev *pdev = kc_adpt->pdev;		\
