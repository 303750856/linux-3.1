...
#define DRV_VERSION "1.0.1.14"
char atl1c_driver_name[] = "ATL1C";
char atl1c_driver_version[] = DRV_VERSION;
#define PCI_DEVICE_ID_ATTANSIC_L1C      0x1067

#define PCI_DEVICE_ID_ATTANSIC_L1C_1      0x1063
/*
...
static void atl1c_set_mac_type(struct atl1c_hw *hw)
{
	...
	case DEV_ID_ATL1D_2_0:
		hw->nic_type = athr_l1d_2; /* just use l1d configure */
		break;
	default:
		break;
	}
...
static int __devinit atl1c_sw_init(struct atl1c_adapter *adapter)
{
	...
	/* clear Vendor Message for L1d & L2cb. added by cloud*/
	if (hw->device_id == DEV_ID_ATL1D || hw->device_id ==DEV_ID_ATL2C_B) {
		AT_READ_REG(hw, REG_DMA_DEBUG, &dma_debug_data);
		dma_debug_data &= ~DMA_DEBUG_VENDOR_MSG;
		AT_WRITE_REG(hw, REG_DMA_DEBUG, dma_debug_data);
	}
...
static void atl1c_configure_rx(struct atl1c_adapter *adapter)
{
	...
	if (hw->ctrl_flags & ATL1C_ASPM_CTRL_MON &&
			(hw->link_cap_flags & LINK_CAP_SPEED_1000)) {
		rxq_ctrl_data &= ~(ASPM_THRUPUT_LIMIT_MASK << ASPM_THRUPUT_LIMIT_SHIFT);
		if (hw->nic_type == athr_l1d_2)
			rxq_ctrl_data |= ASPM_THRUPUT_LIMIT_NO << ASPM_THRUPUT_LIMIT_SHIFT;
		else 
			rxq_ctrl_data |= ASPM_THRUPUT_LIMIT_100M << ASPM_THRUPUT_LIMIT_SHIFT;
 ...
 */
static int atl1c_init_hw(struct atl1c_hw *hw)
{
	int ret_val = 0;
	/* FIXME: should not call any more */
	ret_val = atl1c_phy_init(hw);
...
static void atl1c_set_aspm(struct atl1c_hw *hw, bool linkup)
{
	...
	if (linkup) {
		...
		/* enable l0s/l1 anyway, leave 0x68 be */
		pm_ctrl_data |= PM_CTRL_ASPM_L1_EN;
		pm_ctrl_data |= PM_CTRL_ASPM_L0S_EN;
...
#else
		for (i = 0; i < adapter->num_rx_queues; i++)
                	atl1c_clean_rx_irq(adapter, i, &work_done, 0);
#endif
		}
		  ...
		  struct sk_buff *skb, struct atl1c_recv_ret_status *prrs)
{
	/*
 	 * The pid field in RRS in not correct sometimes, so we
         ...
         */
#if 0
	if ((prrs->word3 >>RRS_ERR_L4_CSUM_SHIFT) & RRS_ERR_L4_CSUM_MASK) {
		skb->ip_summed = CHECKSUM_NONE;
	} else {
		if (RRS_PACKET_PROT_IS_IPV4_ONLY(prrs->word3) ||
		    (RRS_PACKET_PROT_IS_IPV6_ONLY(prrs->word3))){
			skb->ip_summed = CHECKSUM_NONE;
		} else
		   ...
		   int *work_done, int work_to_do)
{
	...
	while (1) {
		...
		rrs_word3 = le32_to_cpu(rrs->word3);
		rrs_word0 = le32_to_cpu(rrs->word0);
		if (likely(RRS_RXD_IS_VALID(rrs_word3))) {
			rfd_num = (rrs_word0 >> RRS_RX_RFD_CNT_SHIFT) &
				RRS_RX_RFD_CNT_MASK;
...
rrs_checked:
		...
		length = (rrs_word3 >> RRS_PKT_SIZE_SHIFT) &
				RRS_PKT_SIZE_MASK;
		/* Good Receive */
		if (likely(rfd_num == 1)) {
			rfd_index = (rrs_word0 >> RRS_RX_RFD_INDEX_SHIFT) &
					RRS_RX_RFD_INDEX_MASK;
...
//				length);
		...
		if (unlikely(adapter->vlgrp) && rrs_word3 & RRS_VLAN_INS) {
			u16 vlan;
			vlan_tag = le16_to_cpu(rrs->vlan_tag);
			AT_TAG_TO_VLAN(vlan_tag, vlan);
#if 0
			((rrs->vlan_tag >> 8) & 0xF) |
				 ((rrs->vlan_tag & 0xF0) << 8) |
				 ((rrs->vlan_tag & 0xF) >> 8);
			printk(KERN_EMERG "vlan_tag is %x\n", vlan_tag);
...
#endif	
#else	/* CONFIG_ATL1C_NAPI */
#ifdef NETIF_F_HW_VLAN_TX			
			vlan_hwaccel_rx(skb, adapter->vlgrp, vlan);
#else
			netif_rx(skb);
#endif
#endif
...
static int atl1c_clean(struct napi_struct *napi, int budget)
{
	...
	//atl1c_clean_tx_irq(adapter, atl1c_trans_high);
	//atl1c_clean_tx_irq(adapter, atl1c_trans_normal);
		...
		if (skb->protocol == htons(ETH_P_IP)) {
			...
			if (unlikely(skb->len == hdr_len)) {
				...
				tcp_hdr(skb)->check = ~csum_tcpudp_magic(
							...
							ip_hdr(skb)->daddr,
							0, IPPROTO_TCP, 0);
				tpd_word1 |= 1 << TPD_IPV4_PACKET_SHIFT;
			}
		}
#ifdef NETIF_F_TSO6
		if (skb_shinfo(skb)->gso_type & SKB_GSO_TCPV6) {
			...
			*tpd = atl1c_get_tpd(adapter, type);
			...
			etpd_word1 |= 1 << TPD_LSO_EN_SHIFT;
			etpd_word1 |= 1 << TPD_LSO_VER_SHIFT;			
			etpd->word1 |= cpu_to_le32(etpd_word1);
			tpd_word1 |= 1 << TPD_LSO_VER_SHIFT;
		}
#endif
		tpd_word1 |= 1 << TPD_LSO_EN_SHIFT;
		tpd_word1 |= (skb_transport_offset(skb) & TPD_TCPHDR_OFFSET_MASK) <<
				TPD_TCPHDR_OFFSET_SHIFT;
#if 0
		printk(KERN_EMERG"GSO_SIZE is %d, real size is %d\n",
			skb_shinfo(skb)->gso_size & TPD_MSS_MASK, skb_shinfo(skb)->gso_size);
#endif
		tpd_word1 |= (skb_shinfo(skb)->gso_size & TPD_MSS_MASK) <<
 				TPD_MSS_SHIFT;
		(*tpd)->word1 |= cpu_to_le32(tpd_word1);
		return 0;
	}
#endif
check_sum:
	if (likely(skb->ip_summed == CHECKSUM_PARTIAL)) {
		...
		if (ip_hdr(skb)->protocol == IPPROTO_UDP)
			(*tpd)->word1 |= 1 << TPD_UDP_CSUM_SHIFT;
		if (ip_hdr(skb)->protocol == IPPROTO_TCP)
			(*tpd)->word1 |= 1 << TPD_TCP_CSUM_SHIFT;
		...
		if (unlikely(cso & 0x1)) {
			...
			tpd_word1 |= ((cso >> 1) & TPD_PLOADOFFSET_MASK) <<
					TPD_PLOADOFFSET_SHIFT;
			tpd_word1 |= ((css >> 1) & TPD_CCSUM_OFFSET_MASK) <<
					TPD_CCSUM_OFFSET_SHIFT;
			tpd_word1 |= 1 << TPD_CCSUM_EN_SHIFT;
			(*tpd)->word1 |= cpu_to_le32(tpd_word1);
		}
	}
	return 0;
			   ...
			   struct atl1c_tpd_desc *tpd, enum atl1c_trans_queue type)
{
	...
	//	printk(KERN_INFO "high produce index is %x \n",
	//		tpd_ring->next_to_use & MB_PRIO_PROD_IDX_MASK);
		break;
	case atl1c_trans_normal:
		...
		//AT_WRITE_REGW(&adapter->hw, (REG_MB_PRIO_PROD_IDX + 2),
			//tpd_ring->next_to_use & MB_PRIO_PROD_IDX_MASK);
	//	printk(KERN_INFO "normal produce index is %x \n",
	//		tpd_ring->next_to_use & 0xffff);
		break;
	default:
		break;
	}
	...
	//data0 = AT_READ_REG(&adapter->hw, REG_MB_PRIO_CONS_IDX);
	//data1 = AT_READ_REG(&adapter->hw, REG_MB_PRIO_PROD_IDX);
	//printk(KERN_INFO "Consume normal: %d high:  %d; Produce normal: %d high: %d; num high %d normal %d\n",
	//	(data0 >> 16) & 0xffff, data0 & 0xffff,  (data1 >> 16) & 0xffff, data1 & 0xffff, i, j);
...
void atl1c_down(struct atl1c_adapter *adapter, unsigned int flag)
{
	struct net_device *netdev = adapter->netdev;
#define ATL1C_DONOT_RESET_MAC 0x01
	...
	if ((flag & ATL1C_DONOT_RESET_MAC) == 0) {
...
#ifdef CONFIG_PM
#if 0
static int atl1c_set_hib(struct atl1c_hw *hw, int enable)
{
	u32 ctrl_data;
	u16 phy_data;

	AT_READ_REG(hw, REG_SERDES_LOCK, &ctrl_data);
	if (enable)
		ctrl_data |= SERDES_PYH_CLK_SLOWDOWN;
	else
		ctrl_data &= ~SERDES_PYH_CLK_SLOWDOWN;
	AT_WRITE_REG(hw, REG_SERDES_LOCK, ctrl_data);
	
	AT_READ_REG(hw, REG_GPHY_CTRL, &ctrl_data);
	if (enable)
		ctrl_data |= GPHY_CTRL_HIB_EN;
	else
		ctrl_data &= ~GPHY_CTRL_HIB_EN;
	AT_WRITE_REG(hw, REG_GPHY_CTRL, ctrl_data);
	
	atl1c_write_phy_reg(hw, MII_DBG_ADDR, 0xB);
	if (atl1c_read_phy_reg(hw, MII_DBG_ADDR, &phy_data) != 0)
		return -1;
	if (enable)
		phy_data |= 0x8000;
	else
...
#endif
int atl1c_suspend(struct pci_dev *pdev, pm_message_t state)
{
	...
	if (netif_running(netdev)) {
		WARN_ON(test_bit(__AT_RESETTING, &adapter->flags));
		atl1c_down(adapter, ATL1C_DONOT_RESET_MAC);
	}
...
int atl1c_resume(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct atl1c_adapter *adapter = netdev_priv(netdev);
	u32 pm_data;
...
void atl1c_shutdown(struct pci_dev *pdev)
{
 ...
 */
int __devinit atl1c_probe(struct pci_dev *pdev,
 ...
 * memory.
 */
void __devexit atl1c_remove(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct atl1c_adapter *adapter = netdev_priv(netdev);
 ...
 */
pci_ers_result_t atl1c_io_error_detected(struct pci_dev *pdev,
 ...
 */
pci_ers_result_t atl1c_io_slot_reset(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct atl1c_adapter *adapter = netdev_priv(netdev);
 ...
 */
void atl1c_io_resume(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct atl1c_adapter *adapter = netdev_priv(netdev); 
