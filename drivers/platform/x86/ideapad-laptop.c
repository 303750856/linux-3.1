 ...
 *  ==========================================================
 *     
 *        This modules is modify by linpus MikeMai
 *        version:    v1.3
 *        last date:  2012/12/14
 *    
 *  ==========================================================
 *
 */
...
module_param(no_bt_rfkill, bool, 0664); //mike modify
...
static size_t brightness_read( struct device *dev , struct device_attribute *attr , char *buf , size_t count )
{
        unsigned long now;
...
static size_t brightness_write( struct device *dev , struct device_attribute *attr , char *buf , size_t count )
{
        unsigned long brightness_set;

        sscanf(buf, "%d", &brightness_set);

        if( brightness_set < 0 && brightness_set > 16)
                return -EIO;

        if (write_ec_cmd(ideapad_handle, 0x13, brightness_set))
                return -EIO;

        return count;
}

static DEVICE_ATTR( brightness, 0666, brightness_read , brightness_write);

static size_t brightness_max_read( struct device *dev , struct device_attribute *attr , char *buf , size_t count )
{
        unsigned long now;

        if (read_ec_data(ideapad_handle, 0x11, &now))
                return -EIO;

        return sprintf(buf, "%d\n", now);
}

static DEVICE_ATTR( brightness_max, 0444, brightness_max_read , NULL );

static size_t inverter_read( struct device *dev , struct device_attribute *attr , char *buf , size_t count )
{
        unsigned long now;

        if (read_ec_data(ideapad_handle, 0x18, &now))
                return -EIO;

        return sprintf(buf, "%d\n", now);
}

static size_t inverter_write( struct device *dev , struct device_attribute *attr , char *buf , size_t count )
{
        unsigned long inverter_set;

        sscanf(buf, "%d", &inverter_set);

        if( inverter_set < 0 && inverter_set > 1)
                return -EIO;

        if (write_ec_cmd(ideapad_handle, 0x33, inverter_set))
                return -EIO;

        return count;
}

static DEVICE_ATTR( inverter, 0666, inverter_read , inverter_write);
...
static struct attribute *ideapad_attributes[] = {
	&dev_attr_camera_power.attr,
        ...
        &dev_attr_brightness.attr,      //mike add
        &dev_attr_brightness_max.attr,  //mike add
        &dev_attr_inverter.attr,        //mike add
	NULL
};
